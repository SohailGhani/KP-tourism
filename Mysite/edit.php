...
<?php
  mysql_connect("localhost","root","");
   ...
   $edit = $_GET['edit'];
   $query = "select * from eventsAll where Id = '$edit'";
   $run = mysql_query($query);
   while ($row=mysql_fetch_array($run)){
	    $name = $row['Name'];
       $eventvanue = $row['Vanue'];
       $status = $row['Status'];
        $disc= $row['Desciption'];
...
<body>
  <form action="" method="post">
      ...
      <table align = "center" width="500px" border="5px" style="color:white;">
          ...
          <tr>
              <td>Event_Vanue</td>
              <td><input type="text" name="vanue" value="<?php echo $eventvanue?>"></td>
          </tr>
           <tr>
              <td>Event_status</td>
...
if(isset($_POST['submit'])){
    ...
    $status = $_POST['status'];
    $disc= $_POST['disc'];
     $q1 = "UPDATE eventsAll set Name = '$name', Vanue = '$eventvanue', Status = '$status', Desciption = '$disc'
